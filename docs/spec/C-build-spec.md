...
| Clients | Sales, operations, leadership | Client organizations and client people. Sales flow: Lead → Suspect → Prospect → Customer. Jobs, submissions, MSA/PO on this record. |
| Vendors | Sales, recruiting ops, operations | Supplier organizations and their people; vendor-sourced candidates; vendor MSA/PO |
...
- Tabs on a Candidate: Overview | Skills/Profile | Requirements | Submissions | Interviews | Placements | Communication | Tasks | Notes | Files | Relationships | Activity. Header: Owner, Last outreach, Next Action, Current Status, Active Requirements, Submissions, Interviews, Source, Last Resume. Client people stay on the Clients module.
- Tabs on a Client organization: Overview | People | Requirements | Candidates Submitted | Interviews | Placements | Communication | Tasks | MSA/PO | Vendors | Files | Reports. Overview shows the intelligence fields in A8.
...
- Submit Profile compose: Requirement + Client person + resume + message. Send via Outlook. Creates Submission ID. Wrap-up required.
- Do not reach disables outbound call / WhatsApp / email; UI explains why.
...
| Candidate, Requirement, Client, Client person | Required. Cannot submit “to an organization” with no job. |
...
| Client organization, hiring-manager person | Required |
...
| WhatsApp (later slice) | Organization numbers, delivery | Conversation panel on the person if built. Not used for client submissions. |
...
4. Upsert the hub object (Person, Submission, ActivityEvent, Task). Never silent-merge people.
5. Fan out: Communication, Overview, Activity events, Tasks, Calendar, search index, Dashboard tiles.
6. On failure: exception queue with retry of safe operations only.

## C.8 Privacy and security (POC, not polish)

1. Tenant isolation on every query (people, organizations, requirements, submissions, activity events, files, insights, search).
...
7. Do not reach disables outbound call/WhatsApp/email.
...
| Organization | Has people, requirements, MSA/PO | Name, type Client/Vendor, owner, industry, location |
| Person | Belongs to organization; has activity events | Type/stage, identity, channels, owner, source, DNC, status |
...
| ActivityEvent | Belongs to person / organization / req / submission | Kind, summary, source, external ID, timestamps |
| Task | Belongs to person/organization/requirement | Owner, due, priority, status, source event; used for wrap-up, SLA follow-up, placement check-ins |
| Insight | Links to activity event and optional task | Type, confidence, proposed/accepted/dismissed |
| File / MSA / PO | Belongs to organization | Type, versions, dates, ceiling/utilized/available, access class |
...
| 2 | People/organizations CRUD, ownership banner, Requirements CRUD, unified timeline shell, wrap-up, Candidate Search & Discovery index (current title, previous titles, resume titles, skills) and POC filters. |
| 3 | Submit Profile send + Outlook ingest, Submission lifecycle, VioTalk click-to-call write-back, JNP one-way sync |
| 4 | Client 360 + Candidate 360 + MSA/PO snapshot, Dashboard risks/opportunities, unmatched queues, Candidate search result context (owner, Last outreach, prior submissions, exclude already submitted), UAT on the demonstration spine. |
...
1. A user can search and open Candidates, Clients and Vendors in the three-pane workspace without losing list filters. On Candidates, POC advanced filters work: current title, skills, location, experience, source, owner, availability, Last outreach, and exclude already submitted to the open requirement. Result cards show owner, Last outreach and previous submissions.
2. Submit Profile selects a Requirement and client person, sends through Outlook with no template catalog, and creates a Submission ID on candidate, job and client.
3. An email sent only in Outlook is either linked to a Submission or sits in unmatched review.
4. A VioTalk call writes one activity event by call ID; wrap-up requires Next Action, No Action Required, or Closed.
...
Index, for every candidate: current title; previous titles from work history; titles extracted from the resume/profile text; skills; location; experience; availability; source; owner; Last outreach; next action; submissions (requirement IDs). Tenant-scoped. Unauthorized roles never see restricted snippets.
...
| --- | --- | --- |
| POC — Advanced filters (build now) | Exact/contains filters, no taxonomy required yet | Title (current) + Skills + Location + Experience + Source + Owner + Availability + Last outreach. Also search previous titles and titles appearing in the resume/profile as additional title fields. Combined Title + Skills. Exclude candidates already submitted to the open requirement. Result card: name, current title, skills snippet, location, owner, Last outreach, next action, previous submissions (client + job + stage). Optional simple rank: recency of outreach + skill overlap. |
...
| Later AI — Semantic search | Same objects, no second stack | Natural language: “Find senior Java developers with Spring Boot, AWS and microservices who are available and haven’t already been submitted to this requirement.” Uses title index, skills, availability, Last outreach and submission exclusions already stored. Management NL questions stay in C.15 and share this index. |
